...
const MEDIA_TRENDING_PATH = path.join(__dirname, 'data', 'media-trending.json');
const WECHAT_CACHE_PATH = path.join(__dirname, 'data', 'wechat-articles-cache.json');
...
function safeMemoryResolve(inputPath) {
  if (!inputPath || typeof inputPath !== 'string') {
    throw new Error('invalid path');
  }
  const p = inputPath.trim();

  if (p === 'MEMORY.md') return MEMORY_MD;
...
async function saveConfigJson(newConfig) {
...
function parseClawhubList(text) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      rows.push({ slug: m[1], version: m[2].trim() });
    }
  }
  return rows;
}
...
function parseHumanNumber(raw) {
  ...
  const m = cleaned.match(/^(\d+(?:\.\d+)?)([万亿wkm]?)$/i);
  if (!m) return null;
...
async function saveWechatCache(cache) {
  await fs.mkdir(path.dirname(WECHAT_CACHE_PATH), { recursive: true });
...
async function loadLocalWechatFallback(fetchedAt) {
  ...
  try {
    ...
    const parsed = JSON.parse(raw);
    const wechatArticles = Array.isArray(parsed?.wechat?.articles) ? parsed.wechat.articles : [];
    ...
    // ignore local trending read failure
  }
...
async function fetchWechatFromTavily(query, limit, fetchedAt, apiKey) {
  ...
  if (!response.ok) {
    ...
    throw new Error(detail || `Tavily 请求失败 (${response.status})`);
  }
...
app.get('/api/openclaw/status', async (_req, res) => {
  try {
    const out = await runBin(OPENCLAW_BIN, ['status'], { timeout: 120000 });
    res.json({ ok: true, text: out.stdout || out.stderr || '' });
  } catch (err) {
...
app.get('/api/openclaw/config', async (_req, res) => {
  try {
    ...
    res.json({ ok: true, config, modelPrimary });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err.message || err) });
  }
});

app.put('/api/openclaw/config', async (req, res) => {
  try {
    const incoming = req.body?.config ?? req.body;
    if (!incoming || typeof incoming !== 'object' || Array.isArray(incoming)) {
      return res.status(400).json({ ok: false, error: 'config must be an object' });
    }
    const backupPath = await saveConfigJson(incoming);
...
app.post('/api/openclaw/model-primary', async (req, res) => {
  try {
    const modelPrimary = (req.body?.modelPrimary || '').trim();
    if (!modelPrimary) {
      return res.status(400).json({ ok: false, error: 'modelPrimary is required' });
    }
    ...
    const backupPath = await saveConfigJson(config);
    res.json({ ok: true, backupPath, modelPrimary });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err.message || err) });
  }
});

app.post('/api/openclaw/restart', async (_req, res) => {
  try {
    const out = await runBin(OPENCLAW_BIN, ['gateway', 'restart'], { timeout: 180000 });
    res.json({ ok: true, text: `${out.stdout}${out.stderr}`.trim() });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err.message || err) });
  }
});
...
app.get('/api/skills', async (_req, res) => {
  try {
    const out = await runBin(CLAWHUB_BIN, ['list'], { timeout: 120000 });
    const rows = parseClawhubList(out.stdout);
    res.json({ ok: true, skills: rows, raw: out.stdout });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err.message || err) });
  }
});
...
app.post('/api/skills/install', async (req, res) => {
  try {
    const slug = sanitizeSkillSlug(req.body?.slug);
    if (!slug) return res.status(400).json({ ok: false, error: 'invalid slug' });

    const out = await runBin(CLAWHUB_BIN, ['install', slug, '--force'], { timeout: 300000 });
    res.json({ ok: true, text: `${out.stdout}${out.stderr}`.trim() });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err.message || err) });
  }
...
app.post('/api/skills/update', async (req, res) => {
  try {
    ...
    if (slugInput && typeof slugInput === 'string' && slugInput.trim()) {
      ...
      if (!slug) return res.status(400).json({ ok: false, error: 'invalid slug' });
      args.push(slug, '--force');
    } else {
      args.push('--all', '--force');
    }

    const out = await runBin(CLAWHUB_BIN, args, { timeout: 300000 });
    res.json({ ok: true, text: `${out.stdout}${out.stderr}`.trim() });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err.message || err) });
  }
...
app.delete('/api/skills/:slug', async (req, res) => {
  try {
    const slug = sanitizeSkillSlug(req.params.slug);
    if (!slug) return res.status(400).json({ ok: false, error: 'invalid slug' });

    const out = await runBin(CLAWHUB_BIN, ['uninstall', slug, '--yes'], { timeout: 180000 });
    res.json({ ok: true, text: `${out.stdout}${out.stderr}`.trim() });
  } catch (err) {
    res.status(500).json({ ok: false, error: String(err.message || err) });
  }
...
app.get('/api/memory/file', async (req, res) => {
  try {
    const filePath = safeMemoryResolve(req.query.path);
    const content = await fs.readFile(filePath, 'utf8');
    res.json({ ok: true, path: req.query.path, content });
  } catch (err) {
...
app.put('/api/memory/file', async (req, res) => {
  try {
    ...
    if (typeof content !== 'string') {
      return res.status(400).json({ ok: false, error: 'content must be string' });
    }

    const filePath = safeMemoryResolve(p);
    if (filePath !== MEMORY_MD) {
...
app.post('/api/memory/file', async (req, res) => {
  try {
    const p = req.body?.path;
    const content = req.body?.content ?? '';
    if (typeof content !== 'string') {
...
app.get('/api/media/trending', async (_req, res) => {
  try {
    const raw = await fs.readFile(MEDIA_TRENDING_PATH, 'utf8');
...
app.get('/api/media/wechat/recommendations', async (req, res) => {
  try {
    const q = typeof req.query.q === 'string' ? req.query.q.trim() : '';
    const limit = clampNumber(req.query.limit, WECHAT_MIN_LIMIT, WECHAT_MAX_LIMIT, WECHAT_MIN_LIMIT);
    ...
    const currentEntry =
      cache?.queries && typeof cache.queries === 'object' ? cache.queries[queryKey] : null;
    const cachedItems = dedupeRecommendations(Array.isArray(currentEntry?.items) ? currentEntry.items : []);
    ...
    const cacheHasRichSignals = cachedItems.some((item) => {
      ...
      const hasMetric = Object.values(metrics).some((v) => Number.isFinite(v));
    ...
    if (!items.length) {
      return res.status(503).json({
        ok: false,
        error: '无法获取公众号推荐数据。请检查 Tavily 配置或稍后重试。'
      });
    }
    ...
    return res.status(500).json({
      ok: false,
      error: `公众号推荐接口失败: ${String(err?.message || err)}`
    });
  }
});
